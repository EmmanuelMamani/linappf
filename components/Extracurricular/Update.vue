<template>
  <div>
    <el-button type="warning" @click="open = true" :loading="loading">
      Editar
    </el-button>

    <el-dialog v-model="open" title="Editar Actividad" width="500px" :close-on-click-modal="false" :append-to-body="true">
...
const props = defineProps({
  ...
  extracurricular_id: { type: Number, required: true }
})
...
const form = ref({
  ...
  institution: props.extracurricular.institution || '',
  start_date: props.extracurricular.start_date || '',
  end_date: props.extracurricular.end_date || '',
  topic_id: props.extracurricular.topic_id || null,
})
...
const handleSubmit = () => {
  formRef.value.validate(async (valid) => {
    ...
    try {
      const response = await sanctum(`/api/extracurricular/${props.extracurricular_id}`, {
        method: 'PUT',
        body: formData,
      })

      emit('updated', response)
      ElMessage.success('Actividad actualizada')
      open.value = false
    } catch (e) {
      console.error(e)
